...
export default Ability.extend({
    ...
    canCreate: Ember.computed('sessionService.permissions', function() {
        return (this.get('sessionService.permissions').indexOf('Question.create') >= 0);
    }),
    canEdit: Ember.computed('sessionService.permissions', function() {
        return (this.get('sessionService.permissions').indexOf('Question.edit') >= 0);
    }),
    canShow: Ember.computed('sessionService.permissions', function() {
        return (this.get('sessionService.permissions').indexOf('Question.show') >= 0);
    }),
    canDelete: Ember.computed('sessionService.permissions', function() {
        return (this.get('sessionService.permissions').indexOf('Question.delete') >= 0);
    })
