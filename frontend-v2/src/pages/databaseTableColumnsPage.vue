...
<template>
  <div>
    ...
    <div class="tw-mx-6 tw-my-3" v-if="!loading">
      <AGInput class="tw-bg-white" placeholder="Search columns" v-model:value="q" debounce=300 />
        ...
        <table class="tw-text-default tw-w-full" cellpadding="0" cellspacing="0" >
          <thead>
            <tr class="tw-text-left tw-px-4  tw-text">
              <th class="tw-px-2 tw-py-2 tw-text-auto tw-uppercase tw-font-semibold tw-border-b tw-text-sm tw-text-default" v-for="column in displayColumns"
                :key="column">
                {{column}}
              </th>
            </tr>
          ...
          <tbody>
            <tr class="tw-border-b even:tw-bg-secondary tw-h-[1px] parent-container" :style="parentStyle" v-for="column in filteredColumns" :key="column">
              ...
              <td class="tw-p-2 tw-flex tw-items-center tw-w-full tw-max-w-full tw-gap-2">
                <div class="tw-flex-1" v-if="!column.editMode">{{column.description}}</div>
                <div class="tw-flex-1"> <AGInput v-model:value="column.description" placeholder="Column Description" v-if="column.editMode" /></div>
                <EditIcon size="16" v-if="!column.editMode && currentUser.canEditQuestion" class="tw-cursor-pointer" @click="column.editMode = true"/>                
                <CheckIcon size="16" v-if="column.editMode && currentUser.canEditQuestion" class="tw-cursor-pointer" @click="((column.editMode = false) || true) && saveColumn(column)"/>                
              </td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  components: { AGLoader, WithLoginHeader, ColumnsIcon, AGInput, EditIcon, CheckIcon },

  mixins: [authMixin],

  computed: {
    filteredColumns(){
      if (!this.q){return this.columns}
