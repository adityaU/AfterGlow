...
export default {
  ...
  computed: {
    hasDatabaseAccess() {
      return (
        !this.question?.human_sql?.database?.unique_identifier ||
        this.currentUser?.hasDatabaseAccess(
          this.question?.human_sql?.database?.unique_identifier
        )
      );
    },
  ...
  methods: {
    ...
    saveQuestion() {
      ...
      if (Object.keys(this.question.api_action).length === 0) {
        this.question.api_action = null;
      }

      if (!this.question?.api_action?.url) {
        this.question.api_action = null;
      }
