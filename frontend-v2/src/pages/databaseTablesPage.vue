          ...
          <tbody>
            ...
            >
              <td class="tw-gap-2 tw-p-2">
                <router-link
                  class="tw-flex tw-gap-2 tw-items-center tw-text-primary tw-font-semibold"
                  :to="
                    '/data_references/databases/' +
              ...
              >
                <div class="tw-flex-1" v-if="!table.editMode">
                  {{ table.description }}
                </div>
                <div class="tw-flex-1">
                  <AGInput
                ...
                <EditIcon
                  size="16"
                  v-if="!table.editMode && currentUser.canEditQuestion"
                  class="tw-cursor-pointer"
                  @click="table.editMode = true"
                />
                <CheckIcon
                  size="16"
                  v-if="table.editMode && currentUser.canEditQuestion"
                  class="tw-cursor-pointer"
                  @click="
                    ((table.editMode = false) || true) && saveTable(table)
                  "
                />
              </td>
              <td class="tw-p-2 tw-text-right tw-pr-4 !tw-w-fit">
                <router-link
                  :to="{
                    path: '/questions/new',
                    query: {
                      database_id: $route.params.database_id,
                      table_id: table.id,
                    },
                  }"
