...
const fetchVizResults = async function(vizID, questionID, payload, query, callback, key) {
  ...
  apiV2.post(url, payload, apiConfig(query.token)).then((response) => {
...
const fetchViz = function(vizID, query, callback) {
  ...
  apiV2.get('/visualizations/' + vizID, apiConfig(query.token)).then((response) => {
...
const downloadVizData = function(payload, query, callback) {
  ...
  apiV2.post('/visualizations/create_csv', payload, apiConfig(query.token)).then((response) => {
