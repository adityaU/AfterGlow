...
const fetchTags = async function(callback) {
  ...
  api.get('tags', apiConfig(session.token)).then((response) => {
...
const fetchTagsByIDs = async function(ids, callback) {
  const session = sessionStore()
  api.get('tags?filter=' + JSON.stringify({ id: ids.join(",") }), apiConfig(session.token)).then((response) => {
    callback(response.data.data.map(v => {
      return { ...v.attributes, ...{ id: v.id } }
    }), false)
  }).catch(error => {
...
const createTag = async function(tag, callback) {
  ...
  api.post('tags/', payload, apiConfig(session.token)).then((response) => {
