...
const searchQuestions = async function(query, tag, token, callback) {
  callback(null, true)
  api.get('questions?q=' + query + "&tag=" + tag, apiConfig(token)).then((response) => {
...
const fetchQuestions = async function(token, callback) {
  callback(null, true)
  api.get('questions/', apiConfig(token)).then((response) => {
    const questions = response.data.data.map(d => {
      d.attributes.id = d.id
      return { ...d.attributes, ...d.relationships }
    })
...
const fetchQuestion = async function(id, token, callback) {
  callback(null, true)
  api.get('questions/' + id + '?version=1&share_id=', apiConfig(token)).then((response) => {
...
const fetchQuestionWithShareID = async function(id, shareID, token, callback) {
  callback(null, true)
  api.get('questions/' + id + '?version=1&share_id=' + shareID, apiConfig(token)).then((response) => {
    response.data.data.attributes.id = response.data.data.id
    callback({ ...response.data.data.attributes, ...(response.data.data.relationships || {}) }, false)
  })
}
...
const saveQuestion = async function(id, payload, token, callback) {
  ...
  api.post('questions/', payload, apiConfig(token)).then((response) => {
    response.data.data.attributes.id = response.data.data.id
    callback({ ...response.data.data.attributes, ...(response.data.data.relationships || {}) }, false)
  }).catch(error => {
    console.log(error)
...
export { fetchQuestionResults, fetchQuestion, saveQuestion, addVariable, fetchQuestions, searchQuestions, fetchQuestionWithShareID };
