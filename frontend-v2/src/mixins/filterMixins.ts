...
const formatDate = function(value) {
  return date.formatDate(new Date(value), datetimeFormat)
}
const FilterMixin = {
  methods: {
    getDisplayValues(filter, colDetails) {
      ...
      if (this.getColumnDataType(filter, colDetails) === 'datetime') {
        ...
        if (filter.value && filter.value.type === 'duration') {
          ...
          valDisp = filter.value && filter.value.value && formatDate(filter.value.value)
        }
...
export { FilterMixin, datetimeFormat, formatDate }
