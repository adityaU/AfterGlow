<template>
  <div class="tw-w-full tw-h-full">
    <AGLoader text="Fetching Details" v-if="loading || apiActionsLoading || quesLoading" />
              ...
              <q-menu flat=true transition-show="scale" transition-hide="scale" max-height="400px" :offset="[0, 5]"
                ...
                <div class="card tw-grid tw-grid-cols-1 tw-divider-y">
                  ...
                  <div @click="$emit('update:showRemoveFromDashboardModal', true)" :tabindex="index + 1" v-if="currentUser.canDeleteDashboard"
                    class="tw-cursor-pointer tw--nowrap tw-py-1 tw-px-2 tw-block tw-w-full hover:tw-bg-primary hover:tw-text-white tw-text-ellipsis focus:tw-bg-primary focus:tw-text-white tw-border-b last:tw-border-b-0">
                    <XIcon size=16 class="icon-primary" />
...
export default {
  name: 'AGQues',
  props: ['id', 'queryKey', 'editMode', 'showRemoveFromDashboardModal', 'showHeader', 'headerStyle'],
