  ...
  <Teleport to="body">
          ...
          <div class="tw-w-full tw-flex tw-border-b tw-p-2">
            <div class="tw-flex tw-flex-wrap tw-gap-1 tw-w-full tw-justify-between">
              <div class="tw-text-center">
                ...
                <AGButton @clicked="editor.chain().focus().toggleItalic().run()" class="tw-rounded-none tw-border-l-0"
                ...
                <AGButton class="tw-rounded-none tw-border-l-0" @clicked="editor.chain().focus().toggleStrike().run()"
                ...
                <AGButton class="tw-rounded-none tw-border-l-0"
                  @click="editor.chain().focus().setTextAlign('center').run()" :class="editor.isActive({ textAlign: 'center' })
                      ? 'tw-bg-primary  tw-border-primary'
                      : 'tw-bg-white tw-text-default'
                    ">
                ...
                <AGButton class="tw-rounded-none tw-border-l-0"
                  @click="editor.chain().focus().setTextAlign('right').run()" :class="editor.isActive({ textAlign: 'right' })
                      ? 'tw-bg-primary  tw-border-primary'
                      : 'tw-bg-white tw-text-default'
                    ">
    ...
    " class="tw-rounded-r-full tw-rounded-l-none tw-border-l-0 tw-inline-flex">
              ...
              <div class="tw-text-center">
                ...
                <AGButton class="tw-rounded-none tw-border-l-0"
                  @clicked="editor.chain().focus().toggleOrderedList().run()" :class="editor.isActive('orderedList')
                      ? 'tw-bg-primary  tw-border-primary'
                      : 'tw-bg-white tw-text-default'
                    ">
              ...
              <div class="tw-text-center">
                <AGColorSelector :selectedColor="editor.getAttributes('textStyle').color" @selectColor="(val) => editor.chain().focus().setColor(val).run()
                  " naked="true" class="tw-text-xs tw-mt-[5px]" />
              </div>
            </div>
          </div>
        </template>
        <template #footer>
          <div class="tw-grid tw-grid-cols-12">
            <div class="tw-col-start-11 tw-col-span-2 tw-p-2 tw-text-right">
              ...
              <AGButton @clicked="
                ($emit('save', editor.getHTML()) || true) &&
                $emit('update:open', false)
                " class="tw-bg-primary  tw-py-0 tw-mr-2">
                Save
              </AGButton>
            </div>
          </div>
      ...
      <DeleteNoteModal :query="query" :id="id" @update:id="(val) => $emit('update:id', val)"
        v-model:open="openDeleteNoteModal" @noteDeleted="
          ($emit('removeWidget', widgetID) || true) &&
          $emit('update:open', false)
          " />
    </div>
  </Teleport>
