    ...
    <AGModal class="!tw-fixed" size="small" :show="open" @update:show="(val) => $emit('update:show', val)" :loading="loading"
      ...
      <template #body>
        <div class="tw-p-2 divide-y tw-flex tw-flex-col tw-gap-2">
          ...
          <AGInput placeholder="What do you wanna call it ? " v-model:value="questionLocal.title" />
        </div>
          <div class="tw-flex item-3070-columns">
            <div class="label"> description </div>
          <AGInput placeholder="What does it do? " v-model:value="questionLocal.description" />
        </div>
        </div>
      </template>
      <template #footer>
