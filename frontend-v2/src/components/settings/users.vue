    ...
    <div class="tw-flex tw-flex-col tw-mx-3 tw-gap-1 tw-w-full">
      ...
      <div class="tw-border tw-flex tw-items-center tw-py-2 tw-px-4 tw-gap-2 tw-border-l-4" :class="user.is_deactivated ? 'tw-bg-default/10 tw-border-l-default' : 'tw-bg-white tw-border-l-primary'" v-for="user in users" :key="user">
        ...
        <div class="tw-flex tw-gap-2 tw-text-sm">
          <div class="tw-cursor-pointer tw-font-semibold tw-text-primary tw-uppercase" v-if="!user.is_deactivated" @click="( (editingUser = user)|| true) &&  (openUserEditModal = true)">Edit </div> 
          <div class="tw-cursor-pointer tw-font-semibold tw-text-red-700 tw-uppercase" v-if="!user.is_deactivated" @click="((user.is_deactivated = true) || true) && deactivate(user)" >Deactivate </div> 
          <div class="tw-cursor-pointer tw-font-semibold tw-text-red-700 tw-uppercase" v-if="user.is_deactivated" @click="((user.is_deactivated = false) || true) && activate(user)" >Activate </div> 
        </div>
      </div>
