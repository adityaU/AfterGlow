    ...
    <AGModal class="!tw-fixed" size="small" :show="open" @update:show="(val) => $emit('update:show', val)">
      ...
      <template #body>
        ...
        <div class="tw-p-2 divide-y tw-mt-2" v-if="orgLocal.id">
          ...
          <AGInput v-model:value="openAIAPIKey.value" placeholder="OpenAI API Key" debounce="300" v-if="openAIAPIKey" />

          <div class="label">OpenAI Model Name</div>
          <AGInput v-model:value="openAIModelName.value" placeholder="OpenAI Model Name" debounce="300"
            v-if="openAIModelName" />
          <div class="note">
            You can experiment with different models listed
            <a class="tw-text-primary tw-underline" href="https://platform.openai.com/docs/models/gpt-3" target="_blank">
              here
            </a>
          </div>

          <AGBool v-model:value="usersCanOverrideOpenAIKey.value" v-if="usersCanOverrideOpenAIKey"
            label="Can Users use their own key?" class="label tw-mt-2" />
          <div class="note">
            Enabling this will allow individual editors and admins to use their
            own openAI key. Useful when you are using free version of openAI.
            This can be overriden at organization level.
          </div>
        </div>
      </template>
      <template #footer>
