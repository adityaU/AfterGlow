      ...
      <template #body>
        <div class="tw-p-2 divide-y">
          ...
          <template v-if="teamLocal.id" >
            ...
            <AGSearchSelect :selected="userEmails"
              :queryFunction="searchUsers"
              :options="emailOptions"
              @update:selected="selectUser"
              disableLocalSearch="true"
              description="search for users" class="tw-p-2 tw-pl-0" />

            <div class="label tw-mt-2" >Accessible Databases </div>
            <AGSearchSelect :selected="databaseNames"
              :queryFunction="searchDatabases"
              :options="dbNameOptions"
              @update:selected="selectDb"
              disableLocalSearch="true"
              description="Search for databases" class="tw-p-2 tw-pl-0" />
          </template>
      ...
      <template #footer>
        <div class="tw-flex tw-justify-end tw-gap-1 tw-p-2">
          ...
          <AGButton
            ...
            :class="teamLocal.name ? '' : 'disabled' "
            @clicked="(save() || true) " v-if="!this.teamLocal.id" >
            Create
          </AGButton>
        </div>
      </template>
...
import {createTeam, saveTeam, removeUser, removeDatabase, addUser, addDatabase} from 'src/apis/team'
...
export default {
  ...
  watch: {
    open(){
      if (this.open && !this.usersLoaded){
        fetchUsers(this.setUsers) 
      }
      if (this.open && !this.databasesLoaded){
      ...
      handler(newv, oldv){
        if (!isEqual(this.team, this.teamLocal)){
          if (this.teamLocal.id){
            saveTeam(this.teamLocal, (_t, loading) => { 'pass' })
          }
  ...
  methods: {
    setDatabases(databases, loading){
      ...
      const dbIDs = this.team?.accessible_databases?.data?.map(d => d.id.toString()) || []
      databases?.forEach(d => {
        this.databaseNameIDMapping[d.name] = d
        if (dbIDs.indexOf(d.id.toString()) >= 0){
    ...
    setUsers(users, loading){
      this.allUsers = users
      const userIDs = this.team?.users?.data?.map(u => u.id) || []
      users?.forEach(u => {
    ...
    searchUsers(q){
      this.userQ = q
      this.emailOptions = this.allUsers.filter(u => u.email.toLowerCase().match(q.toLowerCase())).map(u => u.email)
      this.emailOptions.push(q)
    },
    searchDatabases(q){
      this.dbNameOptions = this.allDatabases.filter(d => d.name.toLowerCase.match(q.toLowerCase())).map(u => u.name)
    },
    selectDb(selected){
      ...
      toBeDeleted.forEach(name => {
        ...
        if (db){
          removeDatabase(this.teamLocal.id, db.id, () => {'pass'})
        }
      })
      ...
      toBeAdded.forEach(name => {
        const db = this.databaseNameIDMapping[name]
        if (db){
          addDatabase(this.teamLocal.id, db.id, () => {'pass'})
        }
      })
      this.databases = selected?.map(s => this.databaseNameIDMapping[s]).filter(d => d)
    },
    selectUser(selected){
      ...
      toBeDeleted.forEach(email => {
        ...
        if (user){
          removeUser(this.teamLocal.id, user.id, () => {'pass'})
        }
      })
      ...
      toBeAdded.forEach(email => {
        const user = this.userEmailIDMapping[email]
        if (user){
          addUser(this.teamLocal.id, user.id, () => {'pass'})
        }
      })

      this.users = selected?.map(s => this.userEmailIDMapping[s]).filter(u => u)
    }
  }
}
</script>
