    ...
    <AGModal class="!tw-fixed" size="small" :show="open" @update:show="(val) => $emit('update:show', val)">
      <template #header>
        <div class="tw-flex tw-items-center tw-px-4 tw-gap-1 tw-py-2">
          <UserIcon size="48" class="icon-primary tw-rounded" />
        <div class="tw-text-2xl tw-font-semibold tw-flex-1">
          {{userLocal.full_name || userLocal.email}}
          <div class="note" v-if="userLocal.full_name">{{userLocal.email}}</div>
        </div>

        <div class="tw-cursor-pointer tw-bg-primary tw-text-white tw-text-sm tw-px-2 tw-py-0.5 tw-rounded" v-if="permissionSet">{{permissionSet.name}}
      ...
      <template #body>
        <div class="tw-p-2 divide-y">
          ...
          <AGSearchSelect :selected="teamNames"
            :queryFunction="searchTeams"
            :options="teamNameOptions"
            @update:selected="selectTeam"
            disableLocalSearch="true"
            description="search for teams" class="tw-p-2 tw-pl-0" />
        </div>
        <div class="tw-p-2 divide-y tw-mt-2">
          ...
          <AGInput v-model:value="downloadLimitSettings.value" v-if="downloadLimitSettings" :placeholder="'How many rows can ' + userLocal.full_name + ' download?'" type="number" debounce=300 />
          <div class="note"> This overrides the Global and Organization Level Value </div>

          <AGBool v-model:value="canDownloadSetting.value" v-if="canDownloadSetting" label="Can Download Reports" class="label tw-mt-2" />
          <div class="note">Use this option to disable downloads for this user. This overrides the Global and Organization Level Value </div>
        </div>
      </template>
      <template #footer>
        <div class="tw-flex tw-justify-end tw-gap-1 tw-p-2">
          ...
          <AGButton
            ...
            @clicked="(save() || true) " v-if="!this.userLocal.id" >
            Create
          </AGButton>
        </div>
      </template>
...
import {removeUser,  addUser, fetchTeams} from 'src/apis/team'
import {fetchUserSettings, saveUserSettings, fetchPermissionSets, saveUserWithPS} from 'src/apis/user'
...
export default {
  name: 'AGShareUser',
  components: {AGModal, AGButton, AGSearchSelect, AGInput, AGBool, UserIcon, SelectOptions},
  props: ["open","user"],
    ...
    userLocal:{
      ...
      handler(newv, oldv){
        if (!isEqual(this.user, this.userLocal)){
          this?.userLocal.id && (this.userLocal?.id != this.user.id ) && fetchUserSettings(this.userLocal.id, this.setSettings)
          if (this.userLocal.id){
            saveUser(this.userLocal, (_t, loading) => { 'pass' })
          }
    ...
    downloadLimitSettings: {
      ...
      handler(){
        if (this.downloadLimitSettings?.id){
          saveUserSettings(this.downloadLimitSettings, () => {'pass'})
        }
      }
    },
    canDownloadSetting: {
      ...
      handler(){
        if (this.canDownloadSetting?.id){
          saveUserSettings(this.canDownloadSetting, () => {'pass'})
        }
      }
    },
    user:{
  ...
  mounted(){
    if (this.open){
      ...
      this.userLocal?.id && fetchUserSettings(this.userLocal.id, this.setSettings)
    }
  },
  ...
  methods: {
    ...
    setTeams(teams, loading){
      this.allTeams = teams
      teams?.forEach(u => {
        const userIDs = u.users?.data?.map(u => u.id )
        this.teamIDMapping[u.name.trim()] = u
        if (userIDs.indexOf(this.userLocal.id) >= 0){
          this.teams.push(u)
        }
      })
    },
    ...
    searchTeams(q){
      this.teamNameOptions = this.allTeams.filter(u => u.name.toLowerCase().match(q.toLowerCase())).map(u => u.name)
    },

    saveUserPS(ps) {
      this.permissionSet = ps
      saveUserWithPS(this.userLocal, ps, () => { 'pass'})
    },
    selectTeam(selected){
      ...
      toBeDeleted.forEach(email => {
        const team = this.teamIDMapping[name]
        if (team){
          removeUser(team.id, this.userLocal.id, () => {'pass'})
        }
      })
      ...
      toBeAdded.forEach(name => {
        const team = this.teamIDMapping[name]
        if (team){
          addUser(team.id, this.userLocal.id, () => {'pass'})
        }
      })

      this.teams = selected?.map(s => this.teamIDMapping[s]).filter(u => u)
    }
  }
}
</script>
