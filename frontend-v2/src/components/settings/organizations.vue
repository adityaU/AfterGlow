<template>
  <div class="tw-flex tw-flex-col tw-w-full">
    <div
      ...
      @click="((openOrgNewModal = true) || true) && (editingOrg = null)"
    >
    ...
    <div class="tw-flex tw-flex-col tw-mx-3 tw-gap-2 tw-w-full">
      ...
      <div
        class="tw-border tw-flex tw-items-center tw-py-2 tw-px-4 tw-gap-2 tw-rounded-2xl"
        :class="
          organization.is_deactivated ? 'tw-bg-default/10' : 'tw-bg-white'
        "
        v-for="organization in organizations"
        :key="organization"
      >
        <AffiliateIcon size="32" class="icon-primary tw-rounded-full" />
        ...
        <div class="tw-flex tw-gap-2 tw-text-sm">
          <div
            ...
            @click="
              ((editingOrg = organization) || true) && (openOrgEditModal = true)
            "
          >
            Edit
          </div>
          <div
            ...
            v-if="!organization.is_deactivated"
            @click="(organization.is_deactivated = true) && save(organization)"
          >
            Deactivate
          </div>
          <div
            ...
            v-if="organization.is_deactivated"
            @click="
              ((organization.is_deactivated = false) || true) &&
                save(organization)
            "
          >
            Activate
    ...
    <AGOrgModal
      v-model:open="openOrgNewModal"
      @refresh="fetchOrganizations()"
      :key="editingOrg"
    />
    <AGOrgModal
      v-model:open="openOrgEditModal"
      @refresh="fetchOrganizations()"
      :org="editingOrg"
      :key="editingOrg"
    />
