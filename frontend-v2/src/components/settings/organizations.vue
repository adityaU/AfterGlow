    ...
    <div class="tw-flex tw-flex-col tw-mx-3 tw-gap-1 tw-w-full">
      ...
      <div class="tw-border tw-flex tw-items-center tw-py-2 tw-px-4 tw-gap-2 tw-rounded-sm tw-shadow-sm tw-border-l-4"  :class="organization.is_deactivated ? 'tw-bg-default/10 tw-border-l-default' : 'tw-bg-white tw-border-l-primary'" v-for="organization in organizations" :key="organization">
        ...
        <div class="tw-flex tw-gap-2 tw-text-sm">
          <div class="tw-cursor-pointer tw-font-semibold tw-text-primary tw-uppercase" @click="((editingOrg = organization) || true) && (openOrgEditModal = true)" >Edit </div> 
          <div class="tw-cursor-pointer tw-font-semibold tw-text-red-700 tw-uppercase" v-if="!organization.is_deactivated" @click="(organization.is_deactivated = true) && save(organization)">Deactivate </div> 
          <div class="tw-cursor-pointer tw-font-semibold tw-text-red-700 tw-uppercase" v-if="organization.is_deactivated" @click="((organization.is_deactivated = false) || true) && save(organization)" >Activate </div> 
        </div>
      </div>
      <div class="tw-flex tw-flex-col tw-justify-center tw-items-center tw-bg-white tw-p-4 tw-rounded-sm tw-shadow-sm tw-border " v-if="organizations?.length === 0">
        <div class=""> You do not have any organizations yet. </div> 
        <div class="tw-cursor-pointer tw-font-semibold tw-text-primary tw-uppercase" @click="((openOrgNewModal = true) || true) && (editingOrg = null)" > Add New Organization </div> 
