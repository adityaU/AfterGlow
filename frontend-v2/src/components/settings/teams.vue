<template>
  <div class="tw-flex tw-flex-col tw-w-full">
    <div class="tw-cursor-pointer tw-font-semibold tw-text-primary tw-uppercase tw-text-right tw-py-2"
      v-if="teams?.length > 0" @click="openTeamModal = true"> + Add New Team </div>
    <div class="tw-flex tw-flex-col tw-mx-3 tw-gap-1 tw-w-full">
      <AGLoader class="tw-my-6" text="Fetching teams" v-if="loading" />
      <div class="tw-border tw-flex tw-items-center tw-py-2 tw-px-4 tw-gap-2 tw-bg-white tw-rounded-sm tw-shadow-sm"
        ...
        <div class="tw-flex tw-text-sm tw-flex-col tw-flex-1 tw-items-center">
          <div class="note">{{ team.accessible_databases_count }} Accessible Databases</div>
          <div class="note"> {{ team.users_count }} Users </div>
        </div>
        <div class="tw-flex tw-gap-2 tw-text-sm tw-flex-1  tw-justify-end">
          <div class="tw-cursor-pointer tw-font-semibold tw-text-primary tw-uppercase"
            @click="(openTeamEditModal = true) && (editingTeam = team)">Edit </div>
          <div class="tw-cursor-pointer tw-font-semibold tw-text-red-700 tw-uppercase"
            @click="openDeleteTeamModal = true"> Delete </div>
        </div>
        <AGDeleteEntityModal v-model:open="openDeleteTeamModal" entityName="team" :entityID="team.id"
          @deleted="fetchTeams" />
      </div>
      ...
      <div
        ...
        <div class="tw-cursor-pointer tw-font-semibold tw-text-primary tw-uppercase" @click="openTeamModal = true"> + Add
          New Team </div>
      </div>
    </div>
