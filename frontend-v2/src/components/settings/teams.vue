<template>
  <div class="tw-flex tw-flex-col tw-w-full">
    ...
    <div class="tw-flex tw-flex-col tw-mx-3 tw-gap-1 tw-w-full">
      ...
      <div
        class="tw-border tw-flex tw-items-center tw-py-2 tw-px-4 tw-gap-2 tw-bg-white tw-rounded-2xl"
        v-for="team in teams"
        :key="team"
      >
        <UsersIcon size="32" class="icon-primary tw-rounded-full" />
        ...
        <div class="tw-flex tw-text-sm tw-flex-col tw-flex-1 tw-items-center">
          <div class="note">
            {{ team.accessible_databases_count }} Accessible Databases
          </div>
        ...
        <div class="tw-flex tw-gap-2 tw-text-sm tw-flex-1 tw-justify-end">
          <div
            ...
            @click="(openTeamEditModal = true) && (editingTeam = team)"
          >
            Edit
          </div>
          <div
            class="tw-cursor-pointer tw-font-semibold tw-text-red-700 tw-uppercase"
            @click="openDeleteTeamModal = true"
          >
            Delete
          </div>
        </div>
        <AGDeleteEntityModal
          v-model:open="openDeleteTeamModal"
          entityName="team"
          :entityID="team.id"
          @deleted="fetchTeams"
        />
    ...
    <AGTeamModal
      v-model:open="openTeamEditModal"
      @refresh="fetchTeams()"
      :team="editingTeam"
    />
  </div>
