<template>
  <div
    class="tw-w-full tw-bg-white tw-rounded-2xl tw-border tw-flex tw-min-h-[600px]"
  ...
  <AGLoader
    class="tw-min-h-[450px] tw-bg-white custom-shadow tw-rounded-2xl"
  ...
  >
    <div class="tw-mb-3">
      ...
      <div
        class="tw-flex tw-items-center tw-bg-white tw-py-2 tw-border-x"
      ...
      >
        <QBHorizontalLayout
          ...
          class="tw-mb-[10px] custom-shadow tw-border tw-rounded-sm"
          v-model:queryTerms="currentViz.queryTerms"
          :key="rerenderKey"
          :vizConfig="currentViz?.settings?.general"
          :quesConfig="question && question?.config"
          :hideQueryTerms="apiResponse || !databaseSupportsFilters"
          v-if="currentViz"
        >
          <template #actions>
            <div
              class="tw-cursor-pointer icon-primary tw-p-2"
              @click="
            ...
            <div
              ...
              :class="showDebugInfo ? 'icon-primary' : 'icon-default'"
              @click="
            ...
            <div
              ...
              :class="showApiResponse ? 'icon-primary' : 'icon-default'"
            ...
            <div
              ...
              :class="showSettings ? 'icon-primary' : 'icon-default'"
            ...
            <div
              class="tw-cursor-pointer tw-p-2 icon-default"
              v-if="
                results &&
                results.rows &&
                currentUser.canDownload &&
            ...
            >
              ...
              <q-menu
                ...
                max-height="900px"
                :offset="[0, 5]"
                class="tw-rounded-2xl custom-shadow tw-border tw-overflow-hidden"
                @show="menuShow"
                @keydown="onKeydown"
                auto-close
              >
                <div class="card tw-grid tw-grid-cols-1 tw-divider-y">
                  ...
                  <div
                    ...
                    v-if="currentUser.canCreateDashboard && currentViz?.id"
                    class="tw-cursor-pointer tw-whitespace-nowrap tw-py-1 tw-px-2 tw-block tw-w-full hover:tw-bg-primary hover:tw-text-white tw-text-ellipsis focus:tw-bg-primary focus:tw-text-white tw-border-b last:tw-border-b-0"
                  >
      ...
      <DebugInfo
        ...
        class="tw-bg-white tw-border-x tw-border-b tw-rounded-b-2xl"
    ...
    >
      ...
      >
        ...
        <div class="tw-flex-[1_1_100%] tw-flex tw-flex-col" v-if="!loading">
          ...
          <div class="tw-flex">
            <splitpanes
              class="pane-wrapper default-theme tw-flex !tw-h-full"
              id="results-view"
              ref="chart-parent"
              @resize="settingsPanesize = 100 - $event[0].size"
            >
              <pane
                :size="30"
                ref="chart"
                class="pane pane-left tw-h-full"
                v-if="
                  apiResponse &&
                  showApiResponse &&
                  currentViz &&
      ...
      <pane
        :size="30"
        ref="chart"
        class="pane pane-right !tw-border tw-rounded-2xl custom-shadow"
        v-if="
          showSettings &&
          results?.rows &&
          // results?.rows?.length > 0 &&
      ...
      >
        ...
        >
          ...
          <component
            ...
            :rows="results.rows"
            :colDetails="results.column_details"
            :guessed_formats="results.guessed_formats"
            :queryKey="queryKey"
  ...
  <component
    :is="componentDefs[currentViz.rendererType]['settingsComponent']"
    class="tw-bg-white tw-overflow-auto tw-rounded-sm tw-hidden"
    @settings="(val) => (currentViz.settings[currentViz.rendererType] = val)"
    :columns="results.columns"
    :rows="results.rows"
    :colDetails="results.column_details"
    :guessed_formats="results.guessed_formats"
    ...
    @updateApiActions="$emit('updateApiActions')"
    :key="rerenderKey"
    v-if="!showSettings && results?.rows && !loading"
  />
