...
<template>
  <div class="tw-bg-secondary tw-border-b-2 tw-border-b-primary tw-flex tw-items-center">
    <div
      ...
      v-for="(viz, index) in visualizationsLocal" :key="viz" :class="viz.current
          ? 'tw-bg-primary  hover:tw-bg-primary/80 tw-border-primary tw-border-2 hover:tw-border-primary/80 tw-min-w-[15%]'
      ...
      <span
        ...
        v-if="!viz.edit">{{ viz.name }}</span>
      <span class="edit tw-ml-2 tw-capitalize tw-my-auto hover:tw-text-primary tw-self-end"
        v-if="!viz.edit && (!viz.id || currentUser.canEditQuestion)" @click.stop="(viz.edit = true) && focusInput(index)">
        <EditIcon size="10" />
      </span>
      <span class="delete tw-ml-2 tw-capitalize tw-my-auto hover:tw-text-primary tw-self-end"
        v-if="!viz.edit && (!viz.id || currentUser.canEditQuestion)" @click.stop="$emit('deleteViz', index)">
        <XIcon size="10" />
      </span>
      ...
      <span class="tw-ml-2 tw-capitalize tw-my-auto hover:tw-text-primary tw-self-end" v-if="viz.edit"
        @click="viz.edit = false">
        <CheckIcon size="10" />
      </span>
    </div>
    <div class="tw-p-2 tw-font-semibold tw-text-primary tw-cursor-pointer tw-flex tw-items-center" @click="addNewViz"
    ...
    <div class="tw-flex-[1_1_5%] tw-text-right">
      <slot />
    </div>
  </div>
</template>
