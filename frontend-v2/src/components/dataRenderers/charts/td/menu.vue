<template>
  <q-menu flat=true transition-show="scale" transition-hide="scale" max-height="400px" :offset="[0, 5]"
    ...
    <SelectOptions :options="copyOptions" @select="copyValue" hideSearch=true />
    <div class="tw-border-t" v-if="showFilters">
      <div class="label tw-text-primary tw-px-2 tw-pt-2">Filter</div>
      <BoxSelect :options="boxSelectOperatorOptions" class="tw-text-center tw-my-1"
  ...
  methods: {
    ...
    let value = ""
      if (this.dataType === 'datetime') {
        value = { type: 'datepicker', value: date.formatDate(new Date(this.value), datetimeFormat) }
      }else{
        value = this.value
      }

      let filter = { column: this.column, operator: op, value: value, raw: false }
