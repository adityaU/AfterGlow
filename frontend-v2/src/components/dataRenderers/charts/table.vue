<template>
  <BaseTable v-slot="{columns, rows}" :results="results" :showSettings="showSettings" :settings="settings"
    ...
    <table class="tw-text-default tw-w-full" cellpadding="0" cellspacing="0" >
      ...
      <tbody>
        <tr class="tw-border-b even:tw-bg-secondary tw-h-[1px] parent-container" :style="parentStyle" v-for="row in rows" :key="row">
          <template v-for="el, index in row" :key="el">
            <template v-if="el.type === 'column'">
              <AGTDRenderer :colDetails="colDetails" isColumnObject=true :value="el.value" :columns="columns" :index="index"
                :showFilters="!onDashboard && !results.additional_filters_applied" 
            ...
            <td class=""  v-if="el.type === 'apiAction'">
              <ApiActionLink :link="el" :queryKey="queryKey" :settings="settings && settings.columns" :colDetails="colDetails" />
            </td>
          </template>
        </tr>
