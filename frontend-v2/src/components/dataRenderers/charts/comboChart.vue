...
export default {
  ...
  computed: {
    ...
    chartInformationMessage() {
      const defaultMessage =
        'Please select xAxis and yAxis Columns from settings.';
      const xAxisMissing =
        'Looks like xAxis that you have specified in settings, was not returned in results.';
      const yAxisMissing =
        'Looks like yAxis that you have specified in settings, was not returned in results.';
      if (!this.settings) {
  ...
  methods: {
    ...
    getXType(xaxis) {
      return (
        chartDTMapping[findDataType(this.results.column_details, xaxis)] ||
        'category'
      );
    },
