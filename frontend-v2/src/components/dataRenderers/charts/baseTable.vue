<template v-if="results" >
  <div class="tw-flex tw-flex-col tw-w-full" >
    <AGLocalFilters v-model:filteredResults='filteredResults' :settings="settingsLocal" :results="results" />
    <div class="tw-h-full tw-w-full tw-flex tw-justify-center tw-text-2xl" v-if="rows && rows.length <= 0">
      No Results
    </div>
    <div class="tw-h-full tw-w-full" v-if="rows && rows.length > 0">
      <slot :columns="columns" :rows="rows" v-if="rows && rows.length > 0" :settings="settingsLocal" />

      <div class="pagination tw-my-7 tw-flex tw-justify-center" v-if="lastPageNumber > 1">
    ...
    updateProps(settings) {
      ...
      if (r) {
        ...
        if (eqSet(settingsColumns, allNames) && [...settingsColumns].length > 0) {
          ...
          }).filter(item => item.show).map(item => { return { type: item.apiAction ? 'apiAction' : 'column', name: item.name, displayName: item.displayName} })
            ...
            map((row, index) => {
              ...
              row.forEach((el, i) => {
                columnSettings[r.columns[i]] && (newRow[columnSettings[r.columns[i]].newOrder] = { type: 'column', value: el,
                  formattingSettings: columnSettings[r.columns[i]].formattingSettings })
              })
              this.settingsLocal.columns.filter(item => item.apiAction).forEach((item) => {
                newRow[columnSettings['apiAction_' + item.apiActionID].newOrder] = { type: 'apiAction', value: item.name, details: item.details,
                  formattingSettings: columnSettings['apiAction_' + item.apiActionID]?.formattingSettings,
          ...
          let columnSettings = {}
          this.settingsLocal && this.settingsLocal.columns.forEach((col, i) => {
            columnSettings[col.apiAction ? 'apiAction_' + col.apiActionID : col.name] = col
          })

          rows = r.rows.slice(((this.pageNumber - 1) * this.pageSize), this.pageNumber * this.pageSize).map((row) => {
            return row.map((el, i) => { return { type: 'column', value: el, formattingSettings: columnSettings[r.columns[i]] && columnSettings[r.columns[i]].formattingSettings } })
          });
          this.columns = _.cloneDeep(r.columns).map((col, i) => { return { name: col, type: 'column', displayName:  columnSettings[r.columns[i]] && columnSettings[r.columns[i]].displayName } })
          this.apiActionsQuesLevel && this.apiActionsQuesLevel.filter(aa => (aa.visualization_id === null) || (aa.visualization_id === this.visualizationID)).forEach((apiAction) => {
            ...
            rows.forEach((row, index) => {
              row.push({ type: 'apiAction', value: apiAction.name, details: apiAction,
                formattingSettings: columnSettings['apiAction_' + apiAction.id]?.formattingSettings,
                vars: { cols: r.columns, row: r.rows[index] } })
            })
          })
        }
