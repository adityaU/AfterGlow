<template>
  ...
  >
    <table class="tw-text-default tw-w-full">
      <tbody>
        <tr
          class="tw-border tw-border-default/20 tw-h-[1px]"
          v-for="(row, index) in rows"
          :key="row"
        >
          <template v-for="(el, i) in row" :key="el">
            <template v-if="el.type === 'column'">
              ...
              <td class="tw-px-4 tw-py-2" v-if="i === 0">
                <div
                  class="tw-h-full tw-items-center tw-flex tw-font-semibold tw-uppercase tw-h-inherit"
                >
