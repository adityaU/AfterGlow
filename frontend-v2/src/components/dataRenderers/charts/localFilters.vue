  ...
  <div class="tw-flex tw-p-2">
    <template v-for="f in filters" :key="f" >
      <div class="tw-flex tw-items-center  tw-m-1 tw-cursor-pointer tw-leading-4">
        <div class="tw-flex ">
          <div class="tw-bg-primary/90 tw-text-white tw-px-2 tw-py-1 tw-rounded-l-sm tw-flex tw-gap-2" :class="f.value ? 'tw-rounded-l-sm' : 'tw-rounded-sm' " > 
            <SearchIcon size="16" />
            {{f.displayName || f.name}}
          </div>
          <div class="tw-bg-primary tw-text-white tw-px-2 tw-py-1 tw-rounded-r-sm" v-if="f.value" >
            {{f.value }}
          </div>
...
export default {
  ...
  methods: {
    getFilters(){
      return cloneDeep(this.settings?.columns?.filter(v => v.showLocalFilter) || [])
    },
    filterResults(){
