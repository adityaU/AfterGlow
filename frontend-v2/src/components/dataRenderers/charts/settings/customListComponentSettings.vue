  ...
  <div>
    <div class="tw-pl-6 tw-pr-4 tw-py-2 tw-border-b">
      ...
      <div class="tw-pl-6 tw-mt-2" v-if="showData">
        <AGTableSettings class="tw-bg-white tw-overflow-auto tw-rounded-sm" @settings="(val) => updateSettings(val)"
          :columns="columns" :rows="rows" :colDetails="colDetails" :queryKey="queryKey"
          :apiActionsQuesLevel="apiActionsQuesLevel" :additionalProps="additionalProps" :questionID="questionID"
          :visualizationID="visualizationID" :settings="settingsLocal" @updateApiActions="$emit('updateApiActions')"
          allHidden="true" />
      </div>
    </div>
    <!-- <div class="tw-pl-6 tw-pr-4 tw-py-2 tw-border-b" > -->
    ...
    <div class="tw-pl-6 tw-pr-4 tw-py-2 tw-border-b">
      ...
      <div class="tw-pl-6 tw-mt-2" v-if="showFormattingOptions">
        <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
          ...
          <BoxSelect class="tw-px-0" v-model:selected="settingsLocal.formattingSettings.boxShadow"
        ...
        <template v-if="settingsLocal.formattingSettings.showBorder">
          <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
            ...
            <q-slider class="tw-px-1" v-model="settingsLocal.formattingSettings.borderThickness" step="1" :min="0"
              :max="10" color="primary" label />
          </div>
          <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
            <div class="label">Border Radius:</div>
            <q-slider class="tw-px-1" v-model="settingsLocal.formattingSettings.borderRadius" step="1" :min="0" :max="10"
              color="primary" label />
          </div>
          <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
            <div class="label">Border Color:</div>
            <AGColorSelector naked="true" v-model:selectedColor="settingsLocal.formattingSettings.borderColor
              " />
          </div>
        </template>
        <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
          <div class="label">Background Color:</div>
          <AGColorSelector naked="true" v-model:selectedColor="settingsLocal.formattingSettings.backgroundColor
            " />
        </div>
        <AGSelect class="tw-flex tw-items-center item-3070-columns tw-mb-2" :options="justifyOptions"
          v-model:selected="settingsLocal.formattingSettings.justifyContainer" label="Container Items Alignment"
          hideSearch="true" canNotDeselect="true" />
        <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
          <div class="label">Container Items Gap:</div>
          <q-slider class="tw-px-1" v-model="settingsLocal.formattingSettings.gap" step="0.125" :min="0" :max="10"
            color="primary" label />
        </div>
        <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
          <div class="label">Page Size</div>
          <AGInput placeholder="Page Size" v-model:value="settingsLocal.formattingSettings.pageSize" type="number"
            debounce="300" />
        </div>
    ...
    <Teleport to="#settings-container">
      ...
      <div class="tw-p-2 tw-border tw-my-2 tw-border-t tw-bg-white tw-rounded-2xl" ref="canvas">
        <div class="custom-list-grid-stack grid-stack tw-h-full tw-w-full tw-flex tw-min-h-[100px]" :class="gridClass"
          ref="grid">
          <div
          ...
          <template v-for="widget in widgets" :key="widget">
            <template v-if="widget.show">
              <GridWidget v-bind="widget" v-model:widID="widget.widID" :id="widID(widget)" class="grid-widget">
                <div
                  class="overlay tw-font-semibold tw-absolute tw-left-0 tw-right-0 tw-bottom-0 tw-top-0 tw-flex tw-items-center tw-justify-center tw-gap-2 tw-bg-primary/80 tw-cursor-pointer ">
                  {{ widget.name }}
                  <ArrowsMoveIcon size="16" />
                </div>
                <template v-if="!widget.apiAction && widget.show">
                  <AGTDRenderer :colDetails="colDetails" :value="firstRow[widget.name] && firstRow[widget.name].value
                    " :columns="widgets" :index="index(widget.name)" showFilters="false" isColumnObject="true"
