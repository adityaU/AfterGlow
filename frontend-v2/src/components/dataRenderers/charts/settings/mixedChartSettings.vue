    ...
    <div>
      ...
      <div class=" tw-py-2 tw-px-2">
        <AGSelect @select="(val) => ((settingsLocal.xaxis = val) || true) && (settingsLocal.manualUpdate = true)"
          :selected="settingsLocal.xaxis" :options="columns" label="X-Axis Column" description="Select a column" />
      </div>
        ...
        <div class="" v-if="s.show">
          <div class=" tw-py-2  tw-px-2">
            <AGSelect @select="(val) => ((s.dataColumn = val) || true) && (settingsLocal.manualUpdate = true)"
              :options="columns" :selected="s.dataColumn" label="Y-Axis Column" description="Select a column" />
          </div>
          <div class=" tw-py-2  tw-px-2">
            <AGSelect @select="(val) => updateIndividualSettings(val, settingsLocal, index, 'series', 'chartType')"
          ...
          <div class=" tw-py-2  tw-px-2">
            <ColorSelector @selectColor="(val) => ((s.color = val) || true) && (settingsLocal.manualUpdate = true)"
