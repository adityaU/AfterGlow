  ...
  <div class="tw-w-full custom-component" ref="canvas">
    ...
    <template v-else>
      ...
      <div class="tw-flex tw-flex-col">
        <AGLocalFilters :style="localFiltersStyle" v-model:filteredResults='filteredResults' :results="results"
          :settings="settings" />
        <div class="tw-flex tw-justify-center tw-pt-2 tw-text-2xl tw-h-full tw-items-center" v-if="pagedRows.length <= 0">
          No Results
        </div>

        <div class="tw-flex tw-relative tw-p-2 tw-flex-wrap custom-list-grid-stack static tw-pt-2"
          :style="outerContainerStyle" v-if="pagedRows.length > 0">
              ...
              <div class="tw-relative" :style="relativeContainerStyle">
                <div v-for="w in widgets" :key="{ wss: widgetStaticStyle, ws: widgetStyle(w) }" :style="widgetStyle(w)">
                  <template v-if="!w.apiAction && w.show">
                    <AGTDRenderer :colDetails="colDetails" :value="row[columnIndex(w.name)]" :columns="widgets"
                      :index="index(w.name)" showFilters=false isColumnObject=true noMenu=true cursor="normal"
                      :formattingSettings="w.formattingSettings" />
                  </template>
                  <template v-if="w.apiAction && w.show">
                    <ApiActionLink :link="addVars(w, row)" :queryKey="queryKey" :settings="settings.widgets"
        ...
        <div class="pagination tw-my-7 tw-flex" v-if="lastPageNumber > 1" :style="pageNumbersStyle">
          <div class="tw-cursor-pointer tw-rounded-full" href="#"
            :class="[pageNumber === 1 ? arrowButtonsDisabledClass : arrowButtonsClass]" @click="decrementPageNumber">
            <ChevronsLeftIcon size=16 />
          </div>
          <div class="tw-cursor-pointer tw-rounded-full"
          ...
          <div class="tw-cursor-pointer tw-rounded-full" href="#"
            :class="[pageNumber === lastPageNumber ? arrowButtonsDisabledClass : arrowButtonsClass]"
...
export default {
  ...
  data() {
    ...
    const paginationButtonClass = 'tw-border-2 tw-leading-4 tw-m-auto tw-border-primary hover:tw-bg-primary hover:tw-text-tertiary tw-mx-1 tw-py-2'
    const paginationButtonDisabledClass = 'tw-border-2 tw-leading-4 tw-m-auto tw-border tw-border-default/20 hover:tw-bg-default/20 tw-mx-1 tw-py-2 tw-text-default/50'
