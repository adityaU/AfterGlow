    ...
    <AGModal class="!tw-fixed" size="small" :show="open" @update:show="(val) => $emit('update:show', val)" :loading="loading"
      ...
      <template #body>
        <div class="tw-p-2 divide-y">
          <div class="note">Select email ids and/or teams to share this {{entityName}} with: </div>
          <AGSearchSelect v-model:selected="entityLocal.shared_to"
...
export default {
  ...
  computed: {
    ...
    shareableLink(){
      return `${this.hostName}/${this.entityName.toLowerCase()}s/${this.entity.id}?share_id=${this.entityLocal.shareable_link}`
    }
  },
