<template>
  <staged-menu :stages="stages" :currentStage="filterLocal.currentStage" :show="filterLocal.showMenu">
    <template #header>
      <div class="tw-grid tw-grid-cols-6 tw-border-b">
        <div class="tw-p-2 tw-col-span-5 tw-flex">
          <div class="btn tw-cursor-pointer tw-text-text-onprimary hover:tw-bg-primary/80"
            v-for="(dv, i) in displayValues" :key="dv" :class="displayValues.length == 1
    ...
    <template #S1>
      <SelectOptions :options="columns" :selected="filterLocal.column" :menuShow="menuShow" iconComponent="true" @select="(val) =>
...
export default {
  ...
  data() {
    return {
      ...
      time: null,
      date: null,
      datetimeValueTypes: ['duration', 'datepicker'].map((item) => {
      ...
      durationTenseOptions: ['ago', 'later'].map((item) => {
        return { name: item, value: item };
      }),
      datetimeFormat: datetimeFormat,
      stages: [{ name: 'S1' }, { name: 'S2' }, { name: 'S3' }, { name: 'S4' }],
    };
  },
  ...
  computed: {
    ...
    operatorOptions() {
      return operatorOptions[
        findDataType(this.colDetails, this.filterLocal.column) || 'text'
      ].map((item) => {
        return { name: item, value: item };
      });
    },
    valueOptions() {
      if (this.rows && this.filterLocal.column) {
        return [
          ...new Set(
            this.rows.map((item) => {
              return item[this.columns.indexOf(this.filterLocal.column)];
            })
          ),
        ];
      }
    ...
    shouldShowAddFilter() {
      ...
      if (this.filterLocal.column && this.filterLocal.operator) {
        if (
          ['is NULL', 'is not NULL'].indexOf(this.filterLocal.operator) >= 0
        ) {
          return true;
