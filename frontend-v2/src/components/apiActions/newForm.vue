...
<template>
  <Teleport to="body">
    <AGModal bodyClass="tw-grid tw-overflow-y-auto" :show="open" size="large"
      @update:show="(val) => $emit('update:show', val)" :loading="loading" :loadingMessage="loadingMessage"
      noHeader="true">
      <template #footer>
        <div class="tw-flex tw-justify-end tw-p-2 tw-gap-2">
          <AGButton class="" @click="$emit('update:open', false)">Cancel</AGButton>
          <AGButton class="tw-text-white tw-bg-primary" @click="
      ...
      <template #body>
        <splitpanes class="pane-wrapper default-theme tw-flex" ref="chart-parent">
          <pane :size="70" class="pane pane-left !tw-overflow-auto">
            ...
            <div class="custom-list-grid-stack form-grid-stack grid-stack tw-h-fit tw-w-full tw-flex tw-min-h-[100px]"
              :class="gridClass" ref="grid">
              <div
                  ...
                  <GridWidget v-bind="field" v-model:widID="field.label" :id="field.label" class="grid-widget" :class="editingField === field
                    ...
                    <div class="tw-w-full tw-h-full" v-if="field.type === 'Field'">
                      <div class="tw-flex tw-items-center item-3070-columns tw-w-full tw-px-2">
                        ...
                        <component :is="inputTypesRendererMapping[field.inputType]" v-bind="field.rendererConfiguration">
                        </component>
                      </div>
                      <div class="note tw-ml-[30%] tw-pl-1" v-if="field.helpText">
                        {{ field.helpText }}
                      </div>
                      <div class="note tw-ml-[30%] tw-text-red-700 tw-pl-1" v-if="field.required ||
                    ...
                    <template v-if="field.type === 'Divider'">
                      <template v-if="field.rendererConfiguration.orientation ===
                        'horizontal'
                        ">
                        <div class="tw-flex tw-items-center tw-w-full">
                          <div class="tw-flex-1" :style="{
            ...
            <div
              ...
              :class="gridClass" ref="buttonGrid">
              <div
                class="tw-bg-secondary tw-absolute tw-left-0 tw-right-0 tw-top-0 tw-bottom-0 tw-flex tw-items-center tw-justify-center tw-text-2xl tw-text-default/50">
                Buttons
              </div>
                ...
                <template v-if="field.show && field.type === 'Button'">
                  <GridWidget v-bind="field" v-model:widID="field.label" :id="field.label" class="grid-widget" :class="editingField === field
                    ...
                    <template v-if="field.type === 'Button'">
                      <AGButton class="tw-w-full" :style="{
          ...
          <pane :size="30" ref="chart" class="pane pane-right !tw-border !tw-overflow-auto">
            <div class="tw-bg-white tw-h-full">
              <template v-if="currentStage != 0">
                <div class="tw-flex tw-gap-2 tw-items-center tw-p-2">
                  <ArrowLeftIcon size="16" class="tw-stroke-primary tw-cursor-pointer" @click="
              ...
              <AGStagedContainer :stages="stages" :currentStage="currentStage">
                <template #S1>
                  <div class="tw-p-2 tw-border-b tw-font-semibold tw-text-primary">
                    Form Configuration
                  </div>
                  ...
                  <div class="tw-flex tw-items-center tw-w-full tw-py-2 tw-px-2 tw-border-b last:tw-border-0"
                    v-for="field in fields" :key="field">
                    <div class="tw-flex tw-gap-2 tw-items-center">
                      <div class="text-icon-primary tw-inline-block" v-if="field.type === 'Field'">
                        F
                      </div>
                      <div class="text-icon-primary tw-inline-block tw-bg-red-700" v-if="field.type === 'Button'">
                        B
                      </div>
                      <div class="text-icon-primary tw-inline-block tw-bg-yellow-700" v-if="field.type === 'Divider'">
                        D
                      </div>
                      <div class="text-icon-primary tw-inline-block tw-bg-green-700" v-if="field.type === 'Note'">
                        N
                      </div>
                      <div class="tw-border tw-rounded-2xl tw-py-0.5 tw-leading-4 tw-px-1 tw-cursor-pointer">
                        {{ field.label }}
    ...
    " hideSearch="true" />
                    ...
                    <div class="actions tw-flex tw-justify-end tw-flex-1 tw-gap-2">
                      <div class="tw-cursor-pointer" v-if="field.type === 'Field'">
                        <q-tooltip class="tw-z-[200000]" transition-show="scale" transition-hide="scale">
                          Input Type Configuration
                        </q-tooltip>
                        <CodePlusIcon class="tw-stroke-primary" size="16" @click="
                          ((currentStage = 1) || true) &&
                          (editingField = field)
                          " v-if="field.show" />
                      </div>
                      ...
                      <div class="tw-cursor-pointer" v-if="field.show">
                        ...
                        <EyeIcon size="16" class="tw-stroke-primary" @click="
                  ...
                  <div class="tw-text-right">
                    <div class="tw-w-fit tw-font-semibold tw-text-sm tw-cursor-pointer tw-text-primary tw-p-2">
                      ...
                      <q-menu flat="true" transition-show="scale" transition-hide="scale" max-height="400px"
                        :offset="[0, 5]" class="tw-rounded-2xl tw-border tw-overflow-hidden" @show="menuShow"
                        @keydown="onKeydown">
                        <div class="menu-item" @click="addNewField()" v-close-popup>
                          Field
                        </div>
                        <div class="menu-item" @click="addDivider()" v-close-popup>
                          Divider
                        </div>
                ...
                <template #S2>
                  <div class="tw-p-2 tw-flex tw-flex-col tw-gap-2">
                    ...
                    <div class="tw-flex item-3070-columns tw-items-center" v-if="has('orientation')">
                      ...
                      <AGBoxSelect class="tw-pl-0" v-model:selected="editingField.rendererConfiguration.orientation
                    ...
                    <div class="tw-flex item-3070-columns tw-items-center" v-if="has('options')">
                      ...
                      <div>
                        <AGArrayInput class="tw-pl-0" naked="true" :isObject="editingField.rendererConfiguration.isOptionsObject
                          " v-model:value="editingField.rendererConfiguration.options
    " />
                        <div class="note">
                    ...
                    <div class="tw-flex item-3070-columns tw-items-center" v-if="has('validationType')">
                      ...
                      <AGSelect v-model:selected="editingField.rendererConfiguration.validationType
                        " :options="['email', 'url', 'Custom Regex']" hideSearch="true"
                        description="Select a Validation" />
                    </div>
                    <div class="tw-flex item-3070-columns tw-items-center tw-flex-1" v-if="editingField.type === 'Field' &&
                      editingField.rendererConfiguration.validationType ===
                      'Custom Regex' &&
                      has('validationType')
                      ">
                      <div class="label">Regex:</div>
                      <AGInput class="tw-inline-block" v-model:value="editingField.rendererConfiguration.validationRegex
                        " placeholder="Enter Regex without starting and ending /" debounce="300" />
                    </div>
                    <div class="tw-flex item-3070-columns tw-items-center" v-if="editingField.type === 'Field' &&
                      editingField.rendererConfiguration.validationType ===
                      'Custom Regex' &&
                      ...
                      <AGInput v-model:value="editingField.rendererConfiguration.validationErrorText
                        " placeholder="Enter Error Text to show for failed validation" debounce="300" />
                    </div>
                  </div>
                </template>
              </AGStagedContainer>
