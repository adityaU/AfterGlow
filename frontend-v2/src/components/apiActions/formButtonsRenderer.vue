          ...
          >
            <template v-if="field.type === 'Button'">
              <AGButton
                ...
                @click="reset()"
                v-if="field.originalName === 'cancel'"
              >
                {{ field.label }}
              </AGButton>
              <AGButton
                class="tw-w-full tw-h-full tw-flex tw-items-center tw-justify-center"
                :class="!validForm ? 'tw-opacity-50 tw-cursor-not-allowed' : ''"
                :style="{
                  color: field.rendererConfiguration.textColor,
                  'background-color':
                    field.rendererConfiguration.backgroundColor,
              ...
              >
                <AGApiActionLink
                  :variables="variablesWithFormData"
                  fullWidth="true"
                  :link="strippedLink"
                  :queryKey="queryKey"
                  v-if="validForm"
...
export default {
  ...
  computed: {
    ...
    strippedLink() {
      const submitButtons = this?.form?.fields?.filter(
        (f) => f.originalName === 'submit' && f.show
      );
      ...
      link.details.display_settings = link.details.display_settings
        ? link.details.display_settings
        : {};
