<template>
  ...
  <AGFormModal v-model:open="showForm" :variables="variables" :queryKey="queryKey" :form="link?.details?.display_settings?.form" :apiAction="link" v-if="link?.details?.display_settings?.renderForm" />
</template>
<script>
...
export default {
  ...
  props: ['link',  'queryKey', "settings", 'colDetails'],
  data() {
    return {
      showForm: false,
    }
  ...
  computed: {
    variables(){
      ...
      this.link.vars && this.link.vars.cols && this.link.vars.cols.forEach((c, i) => {
        variables.push({ name: c, value: this.link.vars.row && this.link.vars.row[i] })
        variables.push({name: "formatted:" + c, value: this.formattedValue(c, this.link.vars.row && this.link.vars.row[i])})
      })
  ...
  methods: {
    formattedValue(colName, value){
      ...
      if (!this.settings){return value}
      const setting = this.settings.filter(s => s.name === colName) 
      if (setting.length === 0){return value}
      if (!setting[0].formattingSettings) {return value}
      const formattingSetting = extractFormattingSetting(setting[0].formattingSettings, value, dataType )
