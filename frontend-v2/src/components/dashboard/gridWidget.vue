<template>
  <div
    ...
    :class="isEditing ? '!tw-border-2 !tw-border-primary' : ''"
  ...
  >
    ...
    >
      <component
        :is="compDefs[type]"
        :id="widID"
        @update:id="(val) => $emit('update:widID', val)"
        :widgetID="id"
        :queryKey="queryKey"
        :editMode="editMode"
        :size="size"
        ...
        @update:widgetConfiguration="updateWidget"
...
export default {
  ...
  computed: {
    ...
    showHeader() {
      return this.formattingSettings
        ? this.formattingSettings.showHeader
        : true;
    },
    ...
    widgetStyle() {
      const style = {};
      if (this.isNested) {
        style['height'] = this.h * 10 + 'px';
        style['top'] = this.y * 10 + 'px !important';
      }
      return style;
    },
    ...
    widgetContainerStyle() {
      ...
      style['background-color'] = fs.backgroundColor
        ? fs.backgroundColor + ' !important'
        : 'white !important';
      if (fs.borderPosition && fs.borderPosition.indexOf('left') >= 0) {
        style['border-left'] = `${fs.borderThickness || 0}px solid ${
          fs.borderColor || 'transparent'
        }`;
      }
      if (fs.borderPosition && fs.borderPosition.indexOf('right') >= 0) {
        style['border-right'] = `${fs.borderThickness || 0}px solid ${
          fs.borderColor || 'transparent'
        }`;
      }
      if (fs.borderPosition && fs.borderPosition.indexOf('top') >= 0) {
        style['border-top'] = `${fs.borderThickness || 0}px solid ${
          fs.borderColor || 'transparent'
        }`;
      }
      if (fs.borderPosition && fs.borderPosition.indexOf('bottom') >= 0) {
        style['border-bottom'] = `${fs.borderThickness || 0}px solid ${
          fs.borderColor || 'transparent'
        }`;
      }
  ...
  methods: {
    updateWidget(widConf) {
      const wid = cloneDeep(this.widget);
      wid.widgetConfiguration = widConf;
      this.$emit('update:widget', wid);
    },
  },
};
</script>
