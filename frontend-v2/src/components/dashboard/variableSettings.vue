<template>
  <div class="tw-text-primary tw-font-semibold tw-p-2 tw-border-b">
    Variables
  </div>
  ...
  <AGStagedContainer :stages="stages" :currentStage="currentStage">
      ...
      >
        <div class="tw-flex tw-items-center tw-gap-2 tw-flex-1">
          <ArrowLeftIcon
            ...
            @click="
              ((currentStage = 0) || true) &&
                ((editingSettingName = null) || true) &&
                ((editingSetting = null) || true)
            "
          />
          <div
    ...
    <template #S2>
      <div class="tw-p-2 tw-pl-8">
        <div class="tw-flex tw-items-center tw-mb-2 tw-flex-wrap">
          <div class="label tw-w-[30%]">Text Color:</div>
          <AGColorSelector
            naked="true"
            v-model:selectedColor="editingSetting.textColor"
          />
        </div>
        <div class="tw-flex tw-items-center tw-mb-2 tw-flex-wrap">
          <div class="label tw-w-[30%]">Background Color:</div>
          <AGColorSelector
            naked="true"
            v-model:selectedColor="editingSetting.backgroundColor"
          />
        </div>
        <div class="tw-flex tw-items-center tw-mb-2 tw-flex-wrap">
          ...
          <AGColorSelector
            naked="true"
            v-model:selectedColor="editingSetting.borderColor"
          />
        </div>
        <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
          ...
          <q-slider
            class="tw-px-1"
            v-model="editingSetting.borderThickness"
            step="1"
            :min="0"
            :max="15"
            color="primary"
        ...
        <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
          ...
          <q-slider
            class="tw-px-1"
            v-model="editingSetting.fontSize"
            step="0.125"
            :min="0.5"
            :max="4"
            color="primary"
        ...
        <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
          ...
          <BoxSelect
            class="tw-px-0"
            v-model:selected="editingSetting.fontWeight"
        ...
        <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
          ...
          <q-slider
            class="tw-px-1"
            v-model="editingSetting.paddingX"
            step="0.125"
            :min="0.5"
            :max="4"
            color="primary"
        ...
        <div class="tw-flex tw-items-center tw-mb-2 item-3070-columns">
          ...
          <q-slider
            class="tw-px-1"
            v-model="editingSetting.paddingY"
            step="0.125"
            :min="0.5"
            :max="4"
            color="primary"
    ...
    <template #S3> Visualization list </template>
    <template #S1>
        ...
        <div class="tw-pb-1">
          <template v-for="vp in variablePanes" :key="vp">
            <div class="tw-flex tw-px-2 tw-py-1 tw-items-center tw-border-b">
              <div
                class="tw-flex tw-gap-2 tw-flex-1 tw-items-center tw-cursor-pointer"
                @click="vp.displayShow = !vp.displayShow"
              >
            ...
            <template v-if="vp.displayShow">
              <div
                class="tw-flex tw-pr-2 tw-py-1 tw-pl-4 tw-items-center tw-border-b"
                v-for="v in possibleVariables"
                :key="v"
              >
                ...
                <EyeIcon
                  class="tw-stroke-primary tw-cursor-pointer"
                  size="16"
                  v-if="index(vp, v.name) >= 0"
                  @click="removeFromList(vp, v.name)"
                />
                <EyeOffIcon
                  class="tw-cursor-pointer"
                  size="16"
                  v-if="index(vp, v.name) < 0"
                  @click="addToList(vp, v)"
                />
              </div>
              <div
                class="tw-flex tw-pr-2 tw-py-1 tw-pl-4 tw-items-center tw-border-b"
              >
                <div class="tw-flex tw-flex-1 tw-gap-2 tw-items-center">
                  ...
                  <div>
                    <div
                      class="tw-border tw-leading-4 tw-rounded-2xl tw-px-2 tw-py-0.5 tw-cursor-pointer"
                    >
                      {{ vp.additionalParams.clearButtonName }}
                    </div>
                    ...
                    >
                      <AGInput
                        placeholder="Enter Clear Button Name"
                        v-model:value="vp.additionalParams.clearButtonName"
                        debounce="300"
                      />
                    </q-menu>
                  </div>
                ...
                <BaselineIcon
                  ...
                  @click="
                    ((currentStage = 1) || true) &&
                      (editingSettingName = 'Clear Button') &&
                      (editingSetting =
                        vp.additionalParams.clearButtonFormatting)
                  "
                />
                <EyeIcon
                  class="tw-stroke-primary tw-cursor-pointer"
                  size="16"
                  v-if="vp.additionalParams.showClearButton"
                  @click="vp.additionalParams.showClearButton = false"
                />
                <EyeOffIcon
                  class="tw-cursor-pointer"
                  size="16"
                  v-if="!vp.additionalParams.showClearButton"
                  @click="vp.additionalParams.showClearButton = true"
                />
              </div>
              <div
                class="tw-flex tw-pr-2 tw-py-1 tw-pl-4 tw-items-center tw-border-b"
              >
                <div class="tw-flex tw-flex-1 tw-gap-2 tw-items-center">
                  ...
                  <div>
                    <div
                      class="tw-border tw-leading-4 tw-rounded-2xl tw-px-2 tw-py-0.5 tw-cursor-pointer"
                    >
                      {{ vp.additionalParams.resetButtonName }}
                    </div>
                    ...
                    >
                      <AGInput
                        placeholder="Enter Reset Button Name"
                        v-model:value="vp.additionalParams.resetButtonName"
                        debounce="300"
                      />
                    </q-menu>
                  </div>
                ...
                <BaselineIcon
                  ...
                  @click="
                    ((currentStage = 1) || true) &&
                      (editingSettingName = 'Reset Button') &&
                      (editingSetting =
                        vp.additionalParams.resetButtonFormatting)
                  "
                />
                <EyeIcon
                  class="tw-stroke-primary tw-cursor-pointer"
                  size="16"
                  v-if="vp.additionalParams.showResetButton"
                  @click="vp.additionalParams.showResetButton = false"
                />
                <EyeOffIcon
                  class="tw-cursor-pointer"
                  size="16"
                  v-if="!vp.additionalParams.showResetButton"
                  @click="vp.additionalParams.showResetButton = true"
                />
              </div>
              <div
                class="tw-flex tw-pr-2 tw-py-1 tw-pl-4 tw-items-center tw-border-b"
              >
                <div class="tw-flex tw-flex-1 tw-gap-2 tw-items-center">
                  ...
                  <div>
                    <div
                      class="tw-border tw-leading-4 tw-rounded-2xl tw-px-2 tw-py-0.5 tw-cursor-pointer"
                    >
                      {{ vp.additionalParams.buttonName }}
                    </div>
                    ...
                    >
                      <AGInput
                        placeholder="Enter Refresh Button Name"
                        v-model:value="vp.additionalParams.buttonName"
                        debounce="300"
                      />
                    </q-menu>
                  </div>
                ...
                <BaselineIcon
                  ...
                  @click="
                    ((currentStage = 1) || true) &&
                      (editingSettingName = 'Refresh Button') &&
                      (editingSetting =
                        vp.additionalParams.refreshButtonFormatting)
                  "
                ></BaselineIcon>
                <EyeIcon
                  class="tw-stroke-primary tw-cursor-pointer"
                  size="16"
                  v-if="vp.additionalParams.showRefreshButton"
                  @click="vp.additionalParams.showRefreshButton = false"
                />
                <EyeOffIcon
                  class="tw-cursor-pointer"
                  size="16"
                  v-if="!vp.additionalParams.showRefreshButton"
                  @click="vp.additionalParams.showRefreshButton = true"
                />
              </div>
            </template>
          </template>
          <div
            class="tw-font-semibold tw-text-primary tw-p-2"
            @click="addVariablePane"
          >
            + Add Another Variable Row
          </div>
        </div>
      </div>
    </template>
...
import { addVariable, deleteVariable } from 'src/apis/dashboards';
import {
  EyeIcon,
  EyeOffIcon,
  VariableIcon,
...
export default {
  ...
  data() {
    return {
      variablePanes: [newVariablePane([])],
      possibleVariables: this.dashboard?.possible_variables,
      existingVariables: [],
      variablesMap: {},
  ...
  methods: {
    index(vp, name) {
      return vp.additionalParams.variableIDs
        .map((id) => this.variablesMap[id])
        .map((v) => v && v.name)
        .indexOf(name);
    },
