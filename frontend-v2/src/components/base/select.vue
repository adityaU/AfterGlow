<template>
  <span class="">
    ...
    <div
      class="input-border tw-px-4 tw-py-2 border-radius-transition"
      :class="
        roundedClass ? 'tw-rounded-full' : 'tw-rounded-t-3xl tw-border-b-0 '
      "
    >
      <q-menu
        flat="true"
        ...
        max-height="400px"
        class="tw-rounded-b-xl tw-border"
      ...
      >
        <SelectOptions
          ...
          @select="
            (val) =>
              ($emit('select', val) || true) && $emit('update:selected', val)
          "
          :areOptionObjects="areOptionsObject"
      ...
      <div class="tw-flex tw-justify-between tw-items-center">
        ...
        <div class="tw-text-right" v-if="!canNotDeselect">
          <XMarkIcon
            class="tw-w-5 tw-h-5 tw-inline tw-mt-[-1px] tw-cursor-pointer"
            v-if="selected"
            @click="
              (event) => (event.stopPropagation() || true) && clearSelected()
            "
          />
        </div>
      </div>
