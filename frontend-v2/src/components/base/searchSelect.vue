...
<template>
  <div class="">
    ...
    <div class="input-border group" @click="focusOnInput">
      <div class="tw-px-2 tw-py-0.5">
        <q-menu flat=true transition-show="jump-down" transition-hide="jump-up"  max-height="400px"
          ...
          <SelectOptions :options="options" :selected="selectedLocal" :menuShow="menuShow" :multiselect="multiselect" :disableLocalSearch="disableLocalSearch"
            @select="(val) => select(val)" v-model:searchQuery="query"/>
        </q-menu>
        <div class="tw-flex tw-justify-between tw-items-center">
          <div class="tw-flex tw-flex-wrap tw-gap-1"  v-if="selected && selected.length > 0">
            <div class="tw-flex tw-justify-center" v-for="value in selected" :key=value >
              <span class="tw-rounded-l-sm tw-text-white tw-py-0.5 tw-px-2" :class="validate(value) ? 'tw-bg-primary' : 'tw-bg-red-700'">
                {{ value }}
              </span>
              <span class="tw-rounded-r-sm tw-text-white tw-p-0.5 tw-cursor-pointer tw-flex tw-px-2" :class="validate(value) ? 'tw-bg-primary/80' : 'tw-bg-red-700/80'" >
