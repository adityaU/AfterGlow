  ...
  <div>
    <div class="tw-py-2 tw-px-2">
      ...
      <div class="" v-if="!type && valueLocal.type === 'duration'">
        <div class="tw-py-2">
          <AGInput :value="valueLocal.value.durationValue" @inputed="(val) => valueLocal.value.durationValue = val"
            type="number" ref="option_0" placeholder="30" />
          <BoxSelect :options="durationTypeOptions" class="tw-max-w-[400px]" :selected="valueLocal.value.durationType"
            @selected="(val) => valueLocal.value.durationType = val" />
          <BoxSelect :options="durationTenseOptions" :selected="valueLocal.value.durationTense"
            @selected="(val) => valueLocal.value.durationTense = val" />
        </div>
      </div>
      <div class="tw-inline-flex tw-mt-2" v-if="(valueLocal.type === 'datepicker') || type">
        <q-date class="!tw-mr-1" flat=true v-model="valueLocal.value" :mask="datetimeFormat" color="primary"
          v-if="showDate" />
        <q-time class="!tw-ml-1" flat=true v-model="valueLocal.value" :mask="datetimeFormat" color="primary"
          v-if="showTime" />
      </div>
      <div class="tw-flex tw-justify-end tw-border-t tw-pt-2">
        <AGButton class="tw-border-primary tw-bg-primary  hover:tw-bg-primary/80"
          @click="$emit('update:value', valueLocal)" v-close-popup>Done</AGButton>
      </div>
    </div>
  </div>
</template>
