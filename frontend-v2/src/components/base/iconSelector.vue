<template>
  <div>
    <template v-if="selectedLocal">
      <div
        class="icon-default tw-inline-flex tw-cursor-pointer"
        v-html="selectedLocal"
      ></div>
    </template>
    <template v-if="!selectedLocal">
      <div class="icon-default tw-inline-flex tw-cursor-pointer">
        Select an Icon
      </div>
    </template>
    ...
    <q-menu
      ...
      max-height="400px"
      :offset="[0, 5]"
      class="tw-rounded-2xl tw-border tw-overflow-hidden tw-p-2"
      @show="menuShow"
      @keydown="onKeydown"
    >
      <div
        class="tw-flex tw-gap-1 tw-pb-2"
        v-for="chunk in defaultIcons"
        :key="chunk"
      >
        <component
          class="icon-default tw-cursor-pointer"
          :is="icon"
          v-for="icon in chunk"
          :key="icon"
          @click="selectedLocal = getIconHtml(icon)"
          v-close-popup
        />
      </div>
...
import {
  ...
  CurrencyPoundIcon,
  CurrencyDollarIcon,
  CurrencyBahrainiIcon,
  CurrencyBahtIcon,
  CurrencyBitcoinIcon,
  CurrencyCentIcon,
  CurrencyDinarIcon,
  CurrencyDollarAustralianIcon,
  CurrencyDogecoinIcon,
  CurrencyFrankIcon,
  CurrencyLiraIcon,
  PhoneIcon,
  PhoneXIcon,
