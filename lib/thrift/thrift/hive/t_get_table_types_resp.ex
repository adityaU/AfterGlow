defmodule(Thrift.Hive.TGetTableTypesResp) do
  _ = "Auto-generated Thrift struct TCLIService.TGetTableTypesResp"
  ...
  defmodule(BinaryProtocol) do
    ...
    def(deserialize(binary)) do
      deserialize(binary, %Thrift.Hive.TGetTableTypesResp{})
    end
    defp(deserialize(<<0, rest::binary>>, %Thrift.Hive.TGetTableTypesResp{} = acc)) do
      {acc, rest}
    end
    ...
    def(serialize(%Thrift.Hive.TGetTableTypesResp{status: status, operation_handle: operation_handle})) do
      [case(status) do
        nil ->
          raise(Thrift.InvalidValueException, "Required field :status on Thrift.Hive.TGetTableTypesResp must not be nil")
        _ ->
