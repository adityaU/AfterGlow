defmodule(Thrift.Hive.TExecuteStatementResp) do
  _ = "Auto-generated Thrift struct TCLIService.TExecuteStatementResp"
  ...
  defmodule(BinaryProtocol) do
    ...
    def(deserialize(binary)) do
      deserialize(binary, %Thrift.Hive.TExecuteStatementResp{})
    end
    defp(deserialize(<<0, rest::binary>>, %Thrift.Hive.TExecuteStatementResp{} = acc)) do
      {acc, rest}
    end
    ...
    def(serialize(%Thrift.Hive.TExecuteStatementResp{status: status, operation_handle: operation_handle})) do
      [case(status) do
        nil ->
          raise(Thrift.InvalidValueException, "Required field :status on Thrift.Hive.TExecuteStatementResp must not be nil")
        _ ->
