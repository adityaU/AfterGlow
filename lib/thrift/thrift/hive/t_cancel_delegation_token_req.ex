defmodule(Thrift.Hive.TCancelDelegationTokenReq) do
  _ = "Auto-generated Thrift struct TCLIService.TCancelDelegationTokenReq"
  ...
  defmodule(BinaryProtocol) do
    ...
    def(deserialize(binary)) do
      deserialize(binary, %Thrift.Hive.TCancelDelegationTokenReq{})
    end
    defp(deserialize(<<0, rest::binary>>, %Thrift.Hive.TCancelDelegationTokenReq{} = acc)) do
      {acc, rest}
    end
    ...
    def(serialize(%Thrift.Hive.TCancelDelegationTokenReq{session_handle: session_handle, delegation_token: delegation_token})) do
      [case(session_handle) do
        nil ->
          raise(Thrift.InvalidValueException, "Required field :session_handle on Thrift.Hive.TCancelDelegationTokenReq must not be nil")
        _ ->
          [<<12, 1::16-signed>> | Thrift.Hive.TSessionHandle.serialize(session_handle)]
      end, case(delegation_token) do
        nil ->
          raise(Thrift.InvalidValueException, "Required field :delegation_token on Thrift.Hive.TCancelDelegationTokenReq must not be nil")
        _ ->
