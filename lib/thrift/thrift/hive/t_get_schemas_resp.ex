defmodule(Thrift.Hive.TGetSchemasResp) do
  _ = "Auto-generated Thrift struct TCLIService.TGetSchemasResp"
  ...
  defmodule(BinaryProtocol) do
    ...
    def(deserialize(binary)) do
      deserialize(binary, %Thrift.Hive.TGetSchemasResp{})
    end
    defp(deserialize(<<0, rest::binary>>, %Thrift.Hive.TGetSchemasResp{} = acc)) do
      {acc, rest}
    end
    ...
    def(serialize(%Thrift.Hive.TGetSchemasResp{status: status, operation_handle: operation_handle})) do
      [case(status) do
        nil ->
          raise(Thrift.InvalidValueException, "Required field :status on Thrift.Hive.TGetSchemasResp must not be nil")
        _ ->
