defmodule(Thrift.Hive.TGetColumnsResp) do
  _ = "Auto-generated Thrift struct TCLIService.TGetColumnsResp"
  ...
  defmodule(BinaryProtocol) do
    ...
    def(deserialize(binary)) do
      deserialize(binary, %Thrift.Hive.TGetColumnsResp{})
    end
    defp(deserialize(<<0, rest::binary>>, %Thrift.Hive.TGetColumnsResp{} = acc)) do
      {acc, rest}
    end
    ...
    def(serialize(%Thrift.Hive.TGetColumnsResp{status: status, operation_handle: operation_handle})) do
      [case(status) do
        nil ->
          raise(Thrift.InvalidValueException, "Required field :status on Thrift.Hive.TGetColumnsResp must not be nil")
        _ ->
