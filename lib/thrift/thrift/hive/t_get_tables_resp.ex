defmodule(Thrift.Hive.TGetTablesResp) do
  _ = "Auto-generated Thrift struct TCLIService.TGetTablesResp"
  ...
  defmodule(BinaryProtocol) do
    ...
    def(deserialize(binary)) do
      deserialize(binary, %Thrift.Hive.TGetTablesResp{})
    end
    defp(deserialize(<<0, rest::binary>>, %Thrift.Hive.TGetTablesResp{} = acc)) do
      {acc, rest}
    end
    ...
    def(serialize(%Thrift.Hive.TGetTablesResp{status: status, operation_handle: operation_handle})) do
      [case(status) do
        nil ->
          raise(Thrift.InvalidValueException, "Required field :status on Thrift.Hive.TGetTablesResp must not be nil")
        _ ->
