defmodule(Thrift.Hive.TGetTypeInfoResp) do
  _ = "Auto-generated Thrift struct TCLIService.TGetTypeInfoResp"
  ...
  defmodule(BinaryProtocol) do
    ...
    def(deserialize(binary)) do
      deserialize(binary, %Thrift.Hive.TGetTypeInfoResp{})
    end
    defp(deserialize(<<0, rest::binary>>, %Thrift.Hive.TGetTypeInfoResp{} = acc)) do
      {acc, rest}
    end
    ...
    def(serialize(%Thrift.Hive.TGetTypeInfoResp{status: status, operation_handle: operation_handle})) do
      [case(status) do
        nil ->
          raise(Thrift.InvalidValueException, "Required field :status on Thrift.Hive.TGetTypeInfoResp must not be nil")
        _ ->
