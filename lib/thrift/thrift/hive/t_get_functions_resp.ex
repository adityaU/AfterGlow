defmodule(Thrift.Hive.TGetFunctionsResp) do
  _ = "Auto-generated Thrift struct TCLIService.TGetFunctionsResp"
  ...
  defmodule(BinaryProtocol) do
    ...
    def(deserialize(binary)) do
      deserialize(binary, %Thrift.Hive.TGetFunctionsResp{})
    end
    defp(deserialize(<<0, rest::binary>>, %Thrift.Hive.TGetFunctionsResp{} = acc)) do
      {acc, rest}
    end
    ...
    def(serialize(%Thrift.Hive.TGetFunctionsResp{status: status, operation_handle: operation_handle})) do
      [case(status) do
        nil ->
          raise(Thrift.InvalidValueException, "Required field :status on Thrift.Hive.TGetFunctionsResp must not be nil")
        _ ->
