defmodule AfterGlow.AutoComplete  do
  ...
  def autocomplete(database_id, prefix) do
    ...
    |> Repo.all()
    |> Enum.map(fn t ->
      %{name: t.readable_table_name, value: t.readable_table_name, meta: "table", score: 1000/(t.name |> String.length)}
    end)
    ...
    columns = (from c in Column,
      ...
      |> Repo.preload(:table)
      |> Enum.map(fn c ->
      %{name: "#{c.table.readable_table_name}.#{c.name}", value:  "#{c.table.readable_table_name}.#{c.name}", meta: "column", score: 1000/(c.name |> String.length)}
    end)
