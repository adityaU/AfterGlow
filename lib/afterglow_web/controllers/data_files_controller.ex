defmodule AfterGlow.DataFilesController do
  ...
  def fetch_and_upload(conn, params) do
    ...
    case params["queryType"] do
      "query_builder" ->
        Async.perform(&CsvTasks.fetch_and_upload/3, [db_record, permit_params(params), conn.assigns.current_user.email])
      "raw" ->
        Async.perform(&CsvTasks.fetch_and_upload/4, [db_record, params["rawQuery"], params["variables"], conn.assigns.current_user.email])
    end
    json conn, %{success: true}
  end
