...
#[component]
pub fn TypeValidationFormRating(
    tv: ReadSignal<Option<TypeValidation>>,
    set_tv: WriteSignal<Option<TypeValidation>>,
    set_error: WriteSignal<bool>,
) -> impl IntoView {
    let (max, max_updater_func, max_error_message, set_max_error_message) =
        base_comp_attributes_with_options!(tv, set_tv, TypeValidation::Rating, max, i32, None)();
    let (default_local, default_updater_func, default_error_message, set_default_error_message) =
        base_comp_attributes!(tv, set_tv, TypeValidation::Rating, default, None)();
    ...
    ) = base_comp_attributes!(tv, set_tv, TypeValidation::Rating, allowed_decimal, false)();

    let _ = watch(
        move || tv.get(),
        move |new, _, _| {
            if let Some(TypeValidation::Rating {
                default,
                decimal_places,
                max,
                ..
