defmodule AfterGlow.Sql.Adapters.Redshift do
  ...
  def create_pool(config) do
    connection_string =
      'Driver={Redshift};Server=#{config["host_url"]};Port=#{config["host_port"]};Database=#{
        config["db_name"]
      };Uid=#{config["username"]};Pwd=#{config["password"]};UseUnicode=1'
  ...
  def opts do
    [timeout: 172800000, pool: DBConnection.Poolboy, pool_timeout: 120_000]
  end
...
#     end)
  ...
  def get_schema(conn) do
    {:ok, _, data} =
      DBConnection.execute(
        conn,
        %ODBC.Query{statement: ~s/select  table_name as table_name,
    ...
    {:ok,
     ...
     |> Enum.map(fn r ->
       r
       |> Map.merge(%{"table_name" => ~s/"#{r["table_schema"]}"."#{r["table_name"]}"/})
       |> Map.delete("table_schema")
     end)
  ...
  def execute_with_stream(pid, query, download_limit, mapper_fn, options) when is_binary(query) do
    ...
    {:ok, _ , results } = DBConnection.execute(pid, %ODBC.Query{statement: query}, [], opts())
