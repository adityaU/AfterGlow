...
defmodule AfterGlow.Sql.Adapters.InfluxDb do
  ...
  def get_schema(conn) do
    ...
    fields =
      (Jason.decode!(fields.body)["results"] |> Enum.at(0))["series"]
      |> format_schema

    {:ok, tags} = http_query(url(conn.config), conn.config["db_name"], "SHOW TAG KEYS")

    tags =
      (Jason.decode!(tags.body)["results"]
       |> Enum.at(0))["series"]
      |> format_schema
  ...
  def extract_columns_and_table(raw_schema) do
    ...
    %{
      ...
      "columns" => values |> Enum.map(fn y -> y |> format_columns end)
    }
  end
  ...
  def format_results(results) do
    series =
      Jason.decode!(results.body)["results"]
      |> Enum.at(0)
      |> Map.fetch("series")
  ...
  defp interpret_results(results) do
    ...
    case results.status_code do
      ...
      _ ->
        {:error, %{message: Jason.decode!(results.body)["error"]}}
    end
  end
end
