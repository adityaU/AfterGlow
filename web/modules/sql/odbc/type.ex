defmodule AfterGlow.ODBC.Type do
  @moduledoc """
  Type conversions.
  """
  ...
  def encode(value, _) when is_binary(value) do
    with utf16 when is_bitstring(utf16) <-
      ...
      _ -> raise %AfterGlow.ODBC.Error{message: "failed to convert string to UTF16LE"}
    end
  end
  ...
  def decode(value, opts) when is_binary(value) do
    if opts[:preserve_encoding] || String.printable?(value) do
      ...
      if is_tuple(value) do
        v = :unicode.characters_to_binary(value, {:utf16, :little}, :unicode)
  ...
  def decode({date, {h, m, s, mi}}, _) do
    {date, {h, m, s}} |> NaiveDateTime.from_erl!({mi})
  end
