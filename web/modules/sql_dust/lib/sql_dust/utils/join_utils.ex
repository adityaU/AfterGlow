defmodule SqlDust.JoinUtils do
  ...
  defp derive_schema_joins(:belongs_to = cardinality, schema1, schema2, association, options) do
    %{
      ...
      path: schema2.path,
      left_key:
        "#{schema2.path |> quote_alias(options)}.#{
          association.primary_key |> quote_alias(options)
        }",
      right_key:
        "#{schema1.path |> quote_alias(options)}.#{
          association.foreign_key |> quote_alias(options)
        }",
  ...
  defp derive_schema_joins(
    ...
    [
      %{
        ...
        left_key:
          "#{quote_alias(schema2.path <> "_bridge_table", options)}.#{
            quote_alias(association.foreign_key, options)
          }",
        right_key:
      ...
      %{
        ...
        path: schema2.path,
        left_key:
          "#{quote_alias(schema2.path, options)}.#{
            quote_alias(association.association_primary_key, options)
          }",
        right_key:
          "#{quote_alias(schema2.path <> "_bridge_table", options)}.#{
            quote_alias(association.association_foreign_key, options)
          }"
      }
    ]
  end
