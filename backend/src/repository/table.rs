...
impl Table {
    ...
    pub fn delete_old_primary_keys(conn: &mut PgConnection, database_id: i64) -> Result<(), Error> {
        conn.transaction::<_, Error, _>(|conn| {
    ...
    pub fn delete_old_foreign_keys(conn: &mut PgConnection, dbid: i64) -> Result<(), Error> {
        conn.transaction::<_, Error, _>(|conn| {
