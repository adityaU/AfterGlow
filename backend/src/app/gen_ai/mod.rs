...
use super::settings::{
    gen_ai::{gen_ai_configuration, GenAIProvider},
};
...
pub const DATABASE_EXPERT_SYSTEM_PROMPT: &str = "You are a database expert. you only return query in the response. only provide naked query without markdown or any other formatting or any other text.You understanmd multiple databases. user will provide database name in the request.user provides a list of columns, use only those columns. user might provide sql query in jinja template form. don't mess up the variables.";
